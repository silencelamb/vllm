...
__global__ void reshape_and_cache_flash_kernel(
  ...
  for (int i = threadIdx.x; i < n; i += blockDim.x) {
    ...
    //     " "head_idx: %d, head_offset: %d, tgt_key_value_idx: %ld, tgt_key:
    //     %f, " "tgt_value: %f\n", blockIdx.x, threadIdx.x, block_idx,
    //     block_offset, head_idx, head_offset, tgt_key_value_idx,
    //     static_cast<float>(tgt_key), static_cast<float>(tgt_value));

    if constexpr (kv_dt == Fp8KVCacheDataType::kAuto) {
      // printf("Storing key and value in cache without scaling\n");
