...
def test_xla_gpu_compilation():
    ...
    try:
        ...
        print(
            f"Debug: depyf version: {depyf.__version__ if hasattr(depyf, '__version__') else 'unknown'}"
        )
        ...
        with depyf.prepare_debug(temp_dir):
            ...
            llm = LLM(
                ...
                compilation_config={
                    "custom_ops": ["none"]
                },  # Disable custom ops - required for XLA GPU to avoid Dynamo errors
        ...
        if len(compiled_codes) == 0:
            ...
            print(
                "This is expected for XLA GPU with compilation_config={'custom_ops': ['none']}."
            )
            print(
                "The model is still being compiled by XLA, just not captured by depyf."
            )
        else:
        ...
        xla_gpu_attn_prefix = (
            "xla_gpu_paged_attention"  # XLA GPU attention implementation
        )
            ...
            if files and not compiled_fns:
                ...
                if "__compiled_fn" in pattern:
                    compiled_fns = sorted(
                        files,
                        key=lambda s: (
                            extract_compiled_index(s) if "__compiled_fn" in s else 0
                        ),
                    )
                    break
        ...
        if len(compiled_fns) >= 2:
            ...
            with open(compiled_fns[0]) as f:
                ...
                print(
                    f"  File 1: {'HAS' if has_kv_cache else 'NO'} kv_cache (expected: NO kv_cache for prefill)"
                )
            ...
            with open(compiled_fns[1]) as f:
                ...
                has_kv_cache = kv_cache_prefix in content
                has_xla_gpu = (
                    xla_gpu_attn_prefix in content or "xla_gpu" in content.lower()
                )
                print(
                    f"  File 2: {'HAS' if has_kv_cache else 'NO'} kv_cache (expected: HAS kv_cache for decode)"
                )
                print(
                    f"  File 2: {'HAS' if has_xla_gpu else 'NO'} XLA GPU attention references"
                )
            ...
            print(
                "Note: XLA GPU with custom_ops disabled may use different compilation paths than TPU."
            )
            print(
                "This is expected behavior when compilation_config={'custom_ops': ['none']} is set."
            )
...
def test_xla_gpu_compilation_simple():
    ...
    sampling_params = SamplingParams(
        temperature=0.0, max_tokens=10  # Use deterministic sampling
    )
