...
def setup_custom_call():
    ...
    func_addr = ctypes.cast(lib.flash_attn_varlen_xla_custom_call, ctypes.c_void_p).value
...
def flash_attn_varlen_xla_impl(
    ...
    is_causal, window_left, window_right, softcap,
...
def flash_attn_varlen_op_fake(
    ...
    out = torch.empty_strided(
        q.shape, q.stride(), dtype=q.dtype, device="meta"
    )
    softmax_lse = torch.empty_strided(
        [q.shape[1], q.shape[0]], [1, q.shape[1]], 
...
def test_torch_compile():
    ...
    def compiled_flash_attn(q, k, v, cu_seqlens_q, cu_seqlens_k, max_seqlen_q, max_seqlen_k):
    ...
    try:
        out, softmax_lse = compiled_flash_attn(q, k, v, cu_seqlens_q, cu_seqlens_k, seqlen, seqlen)
        xm.mark_step()
        xm.wait_device_ops()
...
def test_comparison_with_vllm():
    ...
    k_cuda = torch.randn(total_k, num_heads, head_size, device=cuda_device, dtype=torch.float16)
    v_cuda = torch.randn(total_k, num_heads, head_size, device=cuda_device, dtype=torch.float16)
    ...
    @torch.compile(backend="openxla")
    def compiled_flash_attn(q, k, v, cu_seqlens_q, cu_seqlens_k):
        return torch.ops.xla.flash_attn_varlen_op(
            q, k, v,
            cu_seqlens_q, cu_seqlens_k,
            seqlen_q, seqlen_k,
    ...
    out_xla, lse_xla = compiled_flash_attn(q_xla, k_xla, v_xla, cu_seqlens_q_xla, cu_seqlens_k_xla)
