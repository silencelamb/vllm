...
__global__ void inplace_add_kernel(const float* input, float* output, int size) {
...
void XlaGpuInplaceAdd(cudaStream_t stream, void** buffers, 
...
__global__ void regular_add_kernel(const float* a, const float* b, float* output, int size) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < size) {
...
void XlaGpuRegularAdd(cudaStream_t stream, void** buffers,
                      const char* opaque, size_t opaque_len) {
    int32_t size;
    memcpy(&size, opaque, sizeof(int32_t));
